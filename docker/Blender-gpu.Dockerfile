...
RUN apt-get update --yes --fix-missing && \
    apt-get install --yes --quiet --no-install-recommends \
      ...
      subversion \
      wget 
      # **Here I install wget in order to download my manually downloaded linux using the svn**
...
RUN mkdir lib && \
    ...
    # Here replace the svn instruction by the wget on the link where I stored the manually downloaded linux, decompress it and remove the archive file.
    wget -O linux_centos7_x86_64.tar.xz <link to where you stored the manually downloaded linux compressed as a .tar.xz file> && \
    tar -xf linux_centos7_x86_64.tar.xz && \
    rm -rf linux_centos7_x86_64.tar.xz
...
LABEL Author="KubriCount contributors"
LABEL Title="Blender"
